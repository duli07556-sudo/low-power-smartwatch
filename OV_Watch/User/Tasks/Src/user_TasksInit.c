...
{
	//to increase the LVGL tick      每 1ms 调一次，用来给 LVGL 计时。 是钟表的秒针
	lv_tick_inc(1);   
...
void LvHandlerTask(void *argument)
{
   ...
   while(1)
  {
		...
		lv_task_handler();   //   每 1~10ms 调一次，用来让 LVGL 处理刷新、触摸、动画、定时器。 是看钟表干活的人
    osDelay(1);
	}
}
...
void WDOGFeedTask(void *argument)
{
	//owdg   因为外部看门狗是在 WDOGFeedTask 里才调用WDOG_Enable()使能的，而不是在 main 函数里就开始计时，内部看门狗会系统一启动就开始记时
	WDOG_Port_Init();
  while(1)
  {
		WDOG_Feed();
